...
const demoOrders = [
    {
        id: "ORD-1048",
        customer: "Arjun Nair",
        type: "Dine-in · Table 07",
        date: "2026-07-28T11:42:00",
        status: "Preparing",
        items: [
            { name: "Pub Burger", quantity: 2, price: 10 },
            { name: "Lager Beer", quantity: 2, price: 6 }
        ]
    },
    {
        id: "ORD-1047",
        ...
        type: "Takeaway",
        date: "2026-07-28T11:24:00",
        status: "Pending",
        items: [
            { name: "Buffalo Wings", quantity: 1, price: 9 },
            { name: "Mojito", quantity: 2, price: 7.5 }
        ]
    },
    {
        id: "ORD-1046",
        customer: "Rahul Menon",
        type: "Dine-in · Table 03",
        date: "2026-07-28T10:56:00",
        status: "Ready",
        items: [
            { name: "Grilled Chicken", quantity: 1, price: 14 },
            { name: "French Fries", quantity: 1, price: 4 }
        ]
    },
    {
        id: "ORD-1045",
        ...
        type: "Takeaway",
        date: "2026-07-28T10:18:00",
        status: "Completed",
        items: [
            { name: "Margarita", quantity: 2, price: 8 },
            { name: "Chicken Wings", quantity: 1, price: 9 }
        ]
    },
    {
        id: "ORD-1044",
        customer: "Vishnu Kumar",
        type: "Dine-in · Table 11",
        date: "2026-07-27T21:35:00",
        status: "Completed",
        items: [
            { name: "Pub Burger", quantity: 1, price: 10 },
            { name: "Red Wine", quantity: 2, price: 7 }
        ]
    }
];
...
function loadOrders() {
    ...
    if (!storedOrders.length) return demoOrders.map(normalizeOrder);

    const normalizedOrders = storedOrders.map(normalizeOrder);
    const demoOrderIds = new Set(demoOrders.map((order) => order.id));
    const containsRealOrder = normalizedOrders.some((order) => !demoOrderIds.has(order.id));
    const ordersToDisplay = containsRealOrder
        ? normalizedOrders.filter((order) => !demoOrderIds.has(order.id))
        : normalizedOrders;
