...
async function refreshDashboardFromApi(
    ...
    elements.refreshDashboardBtn.classList.add(
        "loading"
    );
    ...
    try {
        ...
        elements.refreshDashboardBtn.classList.remove(
            "loading"
        );
    }
}
...
function loadCachedDashboardData() {
    dashboardData = {
        orders:
            readCachedArray(CACHE_KEYS.orders),

        inventory:
            readCachedArray(CACHE_KEYS.inventory),

        menu:
            readCachedArray(CACHE_KEYS.menu),
...
function cacheDashboardData() {
    localStorage.setItem(
        CACHE_KEYS.orders,
        JSON.stringify(dashboardData.orders)
    );

    localStorage.setItem(
        CACHE_KEYS.inventory,
        JSON.stringify(dashboardData.inventory)
    );

    localStorage.setItem(
        CACHE_KEYS.menu,
        JSON.stringify(dashboardData.menu)
    );
}
...
function getLowStockItems() {
    return dashboardData.inventory.filter(
        (item) => {
...
function renderSummary() {
    ...
    elements.revenueDescription.textContent =
        `From ${completedOrders.length} completed order${
    ...
    elements.ordersDescription.textContent =
        `${completedOrders.length} completed \u00B7 ${openOrders.length} open`;

    elements.sidebarOrderCount.textContent =
        String(openOrders.length);

    elements.sidebarStockCount.textContent =
    ...
    elements.notificationCount.dataset.count =
        String(alertCount);
}
...
function renderSalesChart() {
    ...
    days.forEach((day) => {
        ...
        track.title =
            `${day.label}: ${formatMoney(day.total)}`;
...
function renderInventoryAlerts() {
    ...
    lowItems.forEach((item) => {
        ...
        detail.textContent =
            `Reorder level: ${minimumStock} ${unit}`;
...
function renderPopularItems() {
    ...
    elements.popularItemsEmptyState
        .classList.toggle(
            "show",
...
function renderDataMode() {
    elements.dataMode.classList.add(
        "show"
    );

    elements.dataMode.classList.toggle(
        "error",
        dashboardData.usingCachedData
    );

    const statusText =
        dashboardData.usingCachedData

            ? "Backend unavailable \u00B7 showing saved browser data"

            : `Live database data \u00B7 updated ${
